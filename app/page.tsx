          ...
          <div className="py-6">
            <div className="flex items-end justify-between">
              ...
              <div className="text-right text-[11px] text-white/60">
                ...
                <div className="font-mono tabular text-pulse-blue mt-0.5">
                  ISO-2026-W18
                </div>
              </div>
            </div>
      ...
      <footer className="bg-white border-t border-panel-border">
        <div className="max-w-[1440px] mx-auto px-8 py-6 flex items-center justify-between text-[11px] text-muted-text">
          ...
          <div className="flex items-center gap-6">
            <span>
              Data sources: <span className="font-mono">PostHog · Stripe · Billing module · Jira · Pardot</span>
            </span>
            <span className="text-protocol-blue">
              Draft for team review — not production data
            </span>
          </div>
        </div>
      </footer>
...
function TabButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`text-left px-5 py-3 rounded-t-sm transition-colors border-b-2 ${
        active
          ? "bg-light-bg border-cyan text-midnight"
          : "border-transparent text-white/60 hover:text-white"
      }`}
    >
      <div className={`text-sm font-medium ${active ? "text-midnight" : ""}`}>
        {label}
      </div>
      <div className={`text-[10px] mt-0.5 ${active ? "text-muted-text" : "text-white/50"}`}>
        {sub}
      </div>
    </button>
  );

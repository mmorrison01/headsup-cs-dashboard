...
export default function PSDeliveryDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Capacity Calendar ──────────────────────────────────────────────── */}
      <Panel title="Capacity Calendar" subtitle="Estimated hours due per assignee each week · green < 20h · amber 20–35h · red > 35h · click to filter">
        ...
        <div className="mt-3 flex items-center gap-4 text-[10px] text-muted-text">
          <span>Click week header or person cell to filter</span>
      ...
      <div className="flex gap-4 items-start">
        ...
        {/* Customer Portfolio sidebar */}
        <div className="w-52 flex-shrink-0">
          <div className="bg-white border border-panel-border rounded-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-panel-border">
              <div className="text-[10px] uppercase tracking-[0.1em] text-muted-text font-semibold">Customers</div>
              <div className="text-[10px] text-muted-text mt-0.5">
                {epicRollups.filter(r => r.open > 0).length} with open issues
              </div>
            </div>
            <div className="overflow-y-auto max-h-[600px]">
              <button
                onClick={() => setFilterEpic("all")}
                className={`w-full text-left px-4 py-2 border-b border-panel-border/50 text-[11px] transition-colors ${filterEpic === "all" ? "bg-pulse-blue/5 font-medium text-midnight" : "text-muted-text hover:bg-subtle"}`}
              >
                All customers
              </button>
              {epicRollups.filter(r => r.open > 0 || r.overdue > 0 || r.blocked > 0).map(row => {
                const isActive = filterEpic === row.epic.key;
                return (
                  <button
                    key={row.epic.key}
                    onClick={() => setFilterEpic(isActive ? "all" : row.epic.key)}
                    className={`w-full text-left px-4 py-2.5 border-b border-panel-border/50 transition-colors ${isActive ? "bg-pulse-blue/5" : "hover:bg-subtle"}`}
                  >
                    ...
                    <div className="flex gap-2 mt-0.5 text-[10px] pl-3">
                      ...
                      {row.overdue > 0 && <span className="text-rose-600">{row.overdue} late</span>}
                      {row.blocked > 0 && <span className="text-rose-600">{row.blocked} blocked</span>}
                    </div>
                  </button>
                );
              })}
              {epicRollups.filter(r => r.open > 0).length === 0 && (
                <div className="px-4 py-6 text-center text-[11px] text-muted-text">No active customers</div>
              )}
            </div>
          </div>
        </div>
            ...
            action={
              <div className="flex items-center gap-3">
                <input
                  ...
                  placeholder="Search…"
                  className="text-[11px] border border-panel-border rounded-sm px-2 py-1 w-36 outline-none focus:border-protocol-blue"
                />
                <select
                  value={filterStatus}
              ...
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {filteredIssues.map(issue => {
                    ...
                    const age = daysAgo(issue.lastUpdated);
                    const ageClass = age > 7 ? "text-rose-600 font-medium" : age > 3 ? "text-amber-600" : "text-muted-text";
                    return (
                      <tr
                        key={issue.key}
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-muted-text max-w-[100px] truncate" title={issue.epicSummary ?? ""}>
                          {issue.epicSummary ?? "—"}
                        </td>
                        ...
                        <td className="px-3 py-2 text-muted-text truncate max-w-[90px]" title={issue.assignee ?? ""}>
                          {issue.assignee?.split(" ")[0] ?? "—"}
                        </td>
                        <td className={`px-3 py-2 text-right tabular-nums ${ageClass}`}>
                          {age}d
                        </td>
                        <td className="px-3 py-2 text-center">
                          <button
                  ...
                  {filteredIssues.length === 0 && (
                    <tr><td colSpan={10} className="px-4 py-8 text-center text-muted-text">No issues match filters</td></tr>
                  )}
                </tbody>
              </table>
            </div>
